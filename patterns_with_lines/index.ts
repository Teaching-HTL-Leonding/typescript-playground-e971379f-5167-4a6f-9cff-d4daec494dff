...
function mouseMoved() {
    ...
    noFill();
    let xscaling = Math.round((mouseX/width)*10);

    for (let i = 0, a = 0; i < width; i++, a+=6) {
    stroke(a, 100, 100);
        push();
        for (let g = 0; g < height; g+=10) {
            line(0, 0, 10*xscaling, 5*xscaling);
            line(0, 10*xscaling, 10*xscaling, 5*xscaling);
            translate(0, 10*xscaling);
        }
        pop();
        translate(10*xscaling, 0);
    }
    resetMatrix();
